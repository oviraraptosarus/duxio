...
export function SignalScanner({ className }: SignalScannerProps) {
  ...
  return (
    <div
      ...
      style={{ "--scanner-x": "72%", "--scanner-y": "24%" } as CSSProperties}
      className={cn(
      ...
      <div className="relative p-4 md:p-5">
        <div className="flex items-center justify-between gap-4 rounded-[1.5rem] border border-white/[0.08] bg-black/35 p-3.5 backdrop-blur transition duration-500 group-hover:border-white/[0.14] group-hover:bg-black/45">
          <div className="flex items-center gap-3">
            <Image src="/duxio-symbol.png" alt="" width={40} height={48} className="h-9 w-auto object-contain" priority />
            <div>
              <Image
        ...
        <div className="relative mt-4 grid grid-cols-8 grid-rows-4 gap-2">
        ...
        <div className="relative z-10 -mt-12 grid gap-3 md:grid-cols-3">
          {[
            ...
            <div
              key={label}
              className="rounded-[1.35rem] border border-white/[0.09] bg-[#0c0f12]/90 p-3.5 shadow-2xl backdrop-blur transition duration-300 hover:-translate-y-1 hover:border-[rgba(244,209,155,0.34)] hover:bg-[#11151a]/95"
            >
              <div className="mono text-[0.62rem] uppercase tracking-[0.2em] text-[var(--ink-subtle)]">{number}</div>
              <div className="mt-4 text-base font-semibold tracking-[-0.03em] text-[var(--ink)]">{label}</div>
              <div className="mt-1 text-xs leading-5 text-[var(--ink-subtle)]">{detail}</div>
            </div>
          ))}
        </div>
        <div className="relative z-10 mt-3 rounded-[1.5rem] border border-[var(--line)] bg-[#08090b]/92 p-4 shadow-2xl backdrop-blur transition duration-500 group-hover:border-white/[0.14] group-hover:bg-[#0b0d10]/94">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <div className="mt-2 text-lg font-semibold tracking-[-0.035em] text-[var(--ink)]">Reply before interest goes cold</div>
            </div>
            <div className="hidden rounded-2xl border border-[var(--line)] bg-white/[0.04] px-4 py-3 text-right sm:block">
              <div className="serif text-2xl leading-none tracking-[-0.05em] text-[var(--signal)]">&lt;5m</div>
              <div className="mono mt-1 text-[0.56rem] uppercase tracking-[0.18em] text-[var(--ink-subtle)]">Target reply</div>
            </div>
          </div>
        </div>
      </div>
