...
export function LeaksSection() {
  return (
    <section
      id="leaks"
      className="border-y border-[var(--line)] bg-black/20 py-16 md:py-24"
    >
      <div className="container-shell">
        ...
        <div className="mt-8 md:mt-12 grid gap-px overflow-hidden rounded-[2rem] border border-[var(--line)] bg-[var(--line)] lg:grid-cols-3">
          {revenueLeaks.map((finding, index) => (
            <Reveal key={finding.title} delay={index * 0.04}>
              <article className="group relative min-h-full overflow-hidden bg-[var(--panel)] p-5 md:p-6 transition duration-300 hover:bg-[var(--panel-2)]">
                ...
                <div className="relative mb-6 md:mb-8 flex items-center justify-between">
                  ...
                  <AlertTriangle
                    size={16}
                    className="text-[var(--copper-2)] transition duration-300 group-hover:scale-110"
                  />
                </div>

                <h3 className="relative text-xl font-semibold leading-7 tracking-[-0.03em] text-[var(--ink)]">
                  {finding.title}
                </h3>
        ...
        <Reveal delay={0.16}>
          <div className="panel mt-6 md:mt-8 rounded-[2rem] p-5 md:p-8">
            <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-3 max-w-3xl text-lg leading-8 text-[var(--ink-muted)]">
                  Duxio finds the highest-impact leak, builds the missing system
                  around that moment, then measures whether the business is
                  capturing more demand with less manual work.
                </p>
              </div>
